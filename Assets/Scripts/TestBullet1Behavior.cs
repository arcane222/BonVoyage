...
public class TestBullet1Behavior : MonoBehaviour //for boss
{
    ...
    void Start()
    {
        for (int i=0 ;i<50;i++) {
            ...
            bulletArray[0, i].name = "test_bullet.clone";
        }
    ...
    IEnumerator Blast()
    {
        ...
        do
        {
            for (int i = 0; i < oneShoting; i++)
            {
                ...
                if (!bulletArray[bulletType, i].GetComponent<TestBullet1>().isMoving())
                {
                    float fx = Mathf.Cos(Mathf.PI * 2 * i / oneShoting);
                    float fy = Mathf.Sin(Mathf.PI * i * 2 / oneShoting);
                    bulletArray[bulletType, i].GetComponent<TestBullet1>().setMoving(true,fx,fy);
